...
class StratifiedLinear(nn.Module):
    ...
    """
    def __init__(self, n_items, ism, fixed_effects=False, 
                 linear_terms=False, covariates=None, 
                 context=False, embedding_dim=5, k=1, lambda_reg=1.0, order_reg=2, 
                 top_k=None, no_ego=True, eps=1e-2, num_ranks=5):
        """
        Initializes the stratified MNL model. 
        Utility can be fixed-effect, linear, or CDM.
        
        Inputs: 
        n_items - total number of items in the choice system modeled
        ...
        covariates - (n_obs, n_items, n_features)-sized array of covariates
        context = if True, context-effects are used in utility, and model is CDM.
        k - number of stratified models to learn
        lambda_reg - weight of Laplacian regularization
        order_reg - order of Laplacian regularization (always 2 for the purpose 
        of this paper)
        """
        super().__init__()
